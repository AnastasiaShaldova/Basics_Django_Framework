...
class Command(BaseCommand):
    def handle(self, *args, **options):
        ...
        # test_products = Product.objects.filter(Q(category__name='Люстры') | Q(category__name='Люстры'))
        #
        ...
        action_expired_condition = Q(order__updated__gt=F('order__created') + action_2__time_delta)

        action_1__price = When(action_1_condition, then=F('product__price') * F('quantity') * action_1__discount)
        action_2__price = When(action_2_condition, then=F('product__price') * F('quantity') * action_2__discount)
        action_expired__price = When(action_expired_condition,
